...
select 
  ...
  count(distinct a.customer_id) as count_retention_customer,
  round(count(distinct a.customer_id) / count(distinct c.customer_id),3) as retention_rate,
from cohort c
  cross join retention_range rr
  left join {{ref("fact_manual__activity")}} a
    on c.customer_id = a.customer_id
      and range_overlaps(a.subscription_range , rr.month_range)
group by 
  1
order by 
  1